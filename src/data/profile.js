...
export const PROJECTS = [
  {
    ...
    highlights: [
      ...
      'Productised into something sellable: a separate packaging repo builds the buyer’s Zalo-bot edition from this source without touching it — it copies the checkout to a temp directory, applies an overlay, launcher and persona seam there, builds, then deletes it, so the source tree is byte-identical afterwards. The build refuses to run against a dirty working tree.',
  ...
  {
    id: 'numerology', name: 'Numerology — Report & Commerce Platform', role: 'Architect / Owner',
    summary: 'A paid numerology report product: a deterministic calculation engine, generated documents, and the commerce funnel around them.',
    problem: 'Sell a generated document. The number has to be computed identically wherever it appears, the report has to arrive as a real file rather than a web page, and the money has to be reconciled from a bank webhook that can fire twice.',
    highlights: [
      'The engine is one module with no DI, shared by the guest teaser and the logged-in analysis, so both build the exact same shape — life path, destiny, soul urge, personal year, pinnacles and essence, composed into 16 report blocks from a content pool rather than free-text.',
      'The deliverable is a document, not a page: docxtemplater and pizzip fill a DOCX template, ConvertAPI renders the PDF, and the file is mailed to the buyer — so what they keep is a file, which is also what makes the product worth paying for.',
      'Payment reconciled from the bank, never from the browser: SePay webhooks pass an IP allowlist and a constant-time key comparison, and a unique index on the SePay id makes processing claim-once, so a retried delivery cannot credit an order twice. Buyers pay by VietQR.',
      'A boot-time check that fails loudly: with SEPAY_WEBHOOK_ENABLED set but TRUST_PROXY unset, Express reports nginx’s loopback address for every request and the IP allowlist rejects all of them. The service logs that at startup instead of leaving it to be found in production.',
      'The funnel is the architecture: a free four-index teaser, then email verification, then the paid report — with affiliate attribution and vouchers priced in between.',
      'Scale: ~85,000 lines of TypeScript, 85 REST routes, 162 frontend components, and 1,166 tests across 97 spec files. Honest note: the readings themselves are authored content — the engine picks and composes them deterministically, it does not generate them.',
    ],
  },
  {
    id: 'loyalty', name: 'Loyalty & Rewards Platform', role: 'Architect / Owner',
    summary: 'Points, prize draws and gift redemption for the company’s customer base — the gamification layer over the shared identity.',
    problem: 'Run a prize draw that customers cannot cheat and the business cannot accidentally bankrupt itself with. The odds live on the server, stock has to hold, and the wheel still has to feel like a wheel.',
    highlights: [
      'The client never picks the prize. The server resolves the draw from per-prize absolute win rates, hands the remaining probability evenly to prizes with no configured rate, normalises at runtime if the rates were authored past 100%, then walks a cumulative distribution — so a misconfigured wheel degrades instead of breaking.',
      'Cost control is part of the draw: each prize carries a daily quantity, exhausted prizes drop out of the pool before the roll, and when everything is gone the wheel falls back to the lowest-value prize rather than failing in the customer’s face.',
      'Every win writes a points transaction, so the balance a customer sees is a ledger rather than a counter — the same records back gift exchange and prize fulfilment.',
      'VNPay integration signs the request with HMAC-SHA512 and verifies vnp_SecureHash on the way back, treating the return URL as untrusted input.',
      'It is a platform, not an app: 110 routes, an M2M public API behind x-api-key, a PRM integration channel, and the shared shops identity it has in common with the auth service and the academy.',
      'Honest ceiling: the draw uses Math.random rather than a CSPRNG, and the daily-quantity check is read-then-write, so a burst of simultaneous spins could exceed a cap. Test coverage is 5 spec files against 16 controllers — the thinnest of the systems I own, and where I would start.',
    ],
  },
  {
  ...
  {
    id: 'harness', name: 'Harness — Repository Guidance for Coding Agents', role: 'Head of Technology',
    summary: 'A self-contained Go binary that installs a working contract into a repository, so a coding agent knows its scope, its gates, and what counts as done.',
    problem: 'Coding agents write code well; that is not where they fail. Dropped into an unfamiliar repository an agent cannot tell an authoritative rule from a local habit, decides product policy that was never its to decide, and reports work finished with nothing to check that against.',
    highlights: [
      'The fix is text in the repository, not a plugin: a working mode chosen before acting, authority gates that stop and ask rather than pick a policy, a completion standard that demands executable or observable evidence, durable memory under docs/plans, and four packaged skills. Nothing is agent-specific, so it outlives whichever tool the team is on.',
      'One executable is the entire distribution. go:embed carries every managed file, and at startup the binary checks the declared inventory against the embedded set, validates each relative path and hashes every file. Changing a single payload byte requires bumping the version — two different payloads must never share one identifier.',
      'Updating guidance without destroying local edits is the real problem: each managed path is evaluated as BASE + LOCAL + UPSTREAM, one-sided changes resolve in memory, genuine text conflicts go to git merge-file --diff3, and an overlap stages frozen inputs, returns exit code 2, and leaves live files and provenance untouched.',
      'Writing into someone else’s repository is the dangerous part, so every mutation takes an exclusive lock, recovers an interrupted transaction before starting a new one, writes and verifies a backup, then commits. The journal carries SHA-256 for every prior byte, and an older schema-1 journal is deliberately refused because it has no integrity metadata worth trusting.',
      'Dependencies point inward: domain owns validated paths, hashes and merge outcomes; application owns install, update and recovery policy behind narrow ports; infrastructure and the CLI sit outside. Nothing in domain or application knows about a filesystem, a process or a terminal — which is why the merge engine could be delegated to git rather than reimplemented.',
      'The binary is the productised form of a change I already owned: Claude Code as the team standard, 37 skills and 26 commands encoding our TypeScript and Go rule sets, a CI-gated design-system plugin, and hooks that re-scan every edit. The outcome I am accountable for is a team that went from 12 to 7 while roughly doubling delivery throughput.',
      'Deliberate non-goals keep it small: no task database, no control plane, no release discovery, no self-replacing executable. Scale: 8,576 lines of non-test Go across 62 files. Honest ceiling: exactly one test file with 20 test functions against all of that, and ARCHITECTURE.md still claims there is no checked-in test source — the thinnest coverage of anything I own, and the first thing I would fix.',
    ],
  },
]
